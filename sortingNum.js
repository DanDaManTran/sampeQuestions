...
//looping through the array and compareing current index and next index number. if the next number is lower then the current one both number will switch. the i will go down to -1 so it can start the loop again. once it goes through the whole array without starting over, that means that the array is sortted
	for(var i = 0;i<numbers.length;i++){
		var switching;
		if(numbers[i]>numbers[i+1]){
			switching=numbers[i];
			numbers[i]=numbers[i+1];
			numbers[i+1]=switching;	
			i=-1;
		}
	}
	console.log(numbers);